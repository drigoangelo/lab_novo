...
class PaginaControllerParent {
    ...
    public function form(){
        if($this->auth() === FALSE)
            return $this->view;
        return new View(PaginaAction::getModuleName().'/Pagina.form.php',$this->response, $this->viewMethod);
    }
    ...
    public function formSubmit(){
        ...
        try {
            ...
            $oPaginaAction->validate($this->request);
            $oPagina = $oPaginaAction->add($this->request, true, true);
        } catch (Exception $ex) {
    ...
    public function admFilter() {
        if($this->auth() === FALSE)
            return $this->view;
        $this->adm();
        return new View(PaginaAction::getModuleName().'/Pagina.admFilter.php',$this->response, $this->viewMethod);
    }
    ...
    public function adm() {
        ...
        $conditions = $oPaginaAction->filterConditions($this->request, $this->response, $orderPageConditions);
        
        # paginacao
        $fields = $oPaginaAction->getAdmFields();
        ...
        $resultsPerPage = ControllerUtil::porPagina($this->request, $this->response, "Pagina");
        
        $conditions = join(" AND ", $conditions);
        $objects = $oPaginaAction->paginatedCollection($fields, ($page * $resultsPerPage), $resultsPerPage, $conditions, $orderSql, FALSE, $paginator);
        ...
        $this->response->set("paginationLinks", Paging::getPageLinks(PaginaAction::getModuleName(), "Pagina","admFilter", $total, $this->gapPerPage, $page, "{$orderPageConditions}{$orderPageLinks}", $this->request->get("porPagina")));
        $this->response->set("perPageLinks", Paging::quantidadePorPagina(PaginaAction::getModuleName(), "Pagina","admFilter", $page, "{$orderPageConditions}{$orderPageLinks}", $this->request->get("porPagina")));
        $this->response->set("currentPage", $page);
        return new View(PaginaAction::getModuleName().'/Pagina.adm.php',$this->response, $this->viewMethod);
    }
    ...
    public function view() {
        ...
        return new View(PaginaAction::getModuleName().'/Pagina.view.php',$this->response, $this->viewMethod);
    }
    ...
    public function edit() {
        ...
        $this->response->set("object",$o);
        return new View(PaginaAction::getModuleName().'/Pagina.edit.php',$this->response, $this->viewMethod);
    }
    ...
    public function editSubmit(){
        ...
        try {
            $oPaginaAction = new PaginaAction();
            $oPaginaAction->validate($this->request,true);
            $oPagina = $oPaginaAction->edit($this->request);
        } catch (Exception $ex) {
            return new View(ExceptionHandler::getMessage($ex),$this->response,'print');
        }
        return new View('OK',$this->response,'print');
    ...
    public function del() {
        ...
        try {
            $id = (int) $this->request->get("id");
            $oPaginaAction = new PaginaAction();
            $oPaginaAction->validateDel($id);
            $oPaginaAction->delPhysical($id);
        } catch (Exception $ex) {
            return new View(ExceptionHandler::getMessage($ex),$this->response,'print');
        }
        return new View('OK',$this->response,'print');
    ...
    public function delSelected() {
        ...
        try {
            ...
            $oPaginaAction->validateDel($ids, false);
            $oPaginaAction->delPhysicalSelected($ids);
        } catch (Exception $ex) {
            return new View(ExceptionHandler::getMessage($ex),$this->response,'print');
        }
        return new View('OK',$this->response,'print');
    ...
    public function relPDF(){
        ...
        $this->adm();
        return new View(PaginaAction::getModuleName().'/Pagina.relPdf.php',$this->response, $method_view);
    }
    ...
    public function relXLS(){
        ...
        $aFieldsStructure = PaginaAction::getFieldsStructure();
		$aFieldsTypeGenial = PaginaAction::getFieldsTypeGenial();
        $aTableHeader = array();
        ...
        Util::relXls($objects, $aTableHeader, "Pagina", "PaginaAction");
    ...
    public function suggest($conditions = array(), $order = 'titulo') {
        ...
        $oPaginaAction = new PaginaAction();
        if ($idValue) {
            $objects = ($o = $oPaginaAction->select($idValue))? array($o) :false;
        } else {
            if ($descValue) {
                if(count($conditions) <= 0)
                    $conditions[] = "o.titulo LIKE '%{$descValue}%' ";
            }
            $objects = $oPaginaAction->paginatedCollection(null, ($page * $limit), $limit, $conditions, $order);
        }        
        return new View( ControllerUtil::objectsToSuggest($oPaginaAction,$objects, $page, 'getId'), $this->response, 'print');
    }
