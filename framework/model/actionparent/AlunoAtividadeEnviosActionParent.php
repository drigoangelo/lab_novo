<?php
require dirname(__FILE__) . '/../doctrine/Entities/AlunoAtividadeEnvios.php';
class AlunoAtividadeEnviosActionParent {
    ...
    public function validateParent(&$request,$edicao = false){
		$aFieldsStructure = AlunoAtividadeEnviosAction::getFieldsStructure(); # estrutura da tabela
...
if(isset($aFieldsStructure["score"]) && !isset($aFieldsStructure["score"]["isFk"]) && $aFieldsStructure["score"]["length"]){
    ...
    public function add($request, $commitable = true, $returnObject = false, $doLog = true) {
        ...
        $oAlunoAtividadeEnvios = new AlunoAtividadeEnvios();
        $oAlunoAtividadeEnvios->setAluno(QueryHelper::verifyObject($Aluno, 'Aluno', $this->em));
	$oAlunoAtividadeEnvios->setAtividade(QueryHelper::verifyObject($Atividade, 'Atividade', $this->em));
	$oAlunoAtividadeEnvios->setArquivo($arquivo);
	$oAlunoAtividadeEnvios->setNome($nome);
	$oAlunoAtividadeEnvios->setTipo($tipo);
	$oAlunoAtividadeEnvios->setScore($score);
	
        try {
            if ($commitable) {
                $this->em->beginTransaction();
            }
            $this->em->persist($oAlunoAtividadeEnvios);
            $this->em->flush($oAlunoAtividadeEnvios);
            $this->addTransaction($oAlunoAtividadeEnvios, $request);
        ...
        if($returnObject === true){
            return $oAlunoAtividadeEnvios;
        }
        return true;
    }

    protected function addTransaction($oAlunoAtividadeEnvios, $request){
    }
    ...
    public function edit($request, $commitable = true, $returnObject = false, $doLog = true) {
        ...
        $oAlunoAtividadeEnvios = $this->em->find('AlunoAtividadeEnvios',array('id' => $id));
        $oAlunoAtividadeEnvios->setAluno(QueryHelper::verifyObject($Aluno, 'Aluno', $this->em));
	$oAlunoAtividadeEnvios->setAtividade(QueryHelper::verifyObject($Atividade, 'Atividade', $this->em));
	$oAlunoAtividadeEnvios->setArquivo($arquivo);
	$oAlunoAtividadeEnvios->setNome($nome);
	...
	$oAlunoAtividadeEnvios->setScore($score);
        ...
        try {
            ...
            $this->em->persist($oAlunoAtividadeEnvios);
            $this->em->flush($oAlunoAtividadeEnvios);
            $this->editTransaction($oAlunoAtividadeEnvios, $request);
    ...
    protected function editTransaction($oAlunoAtividadeEnvios, $request){
    }
    ...
    public function paginatedCollection($fieldsToSelect = null, $page = 0, $resultsPerPage = 10, $conditions = null, $order = null, $distinct = FALSE, &$paginator = null) {
        ...
        if(AlunoAtividadeEnviosAction::manualOrder($query, $order) === FALSE){
            if($order) {
...
if($order == 'Aluno') {
    ...
    public function select($id, $fieldsToSelect = null) {
        ...
        $oAlunoAtividadeEnvios = $query->getQuery()->getOneOrNullResult();
        return $oAlunoAtividadeEnvios;
    }
    ...
    public function delLogical($id, $commitable = true, $doLog = true) {
        ...
        $qb = $this->em->createQueryBuilder();
        $where = QueryHelper::getAndEquals(array('o.id' => $id), $qb);
        $query = $qb->update("AlunoAtividadeEnvios o")
            //->set([nome_atributo], 0)
    ...
    public function delPhysical($id, $commitable = true, $doLog = true) {
        ...
        $qb = $this->em->createQueryBuilder();
        $where = QueryHelper::getAndEquals(array('o.id' => $id), $qb);
        $query = $qb->delete()->from("AlunoAtividadeEnvios", "o")
                ->where( $where )->getQuery();
        try {
            if ($commitable) {
                $this->em->beginTransaction();
    ...
    public static function getEntityName(){
        return 'Atividade do Aluno Envios';
    }

    public static function getTableName(){
        return 'aluno_atividade_envios';
    }
    ...
    public static function getClassName() {
        return 'AlunoAtividadeEnvios';
    }
    
    public static function getBaseUrl() {
        return URL . AlunoAtividadeEnviosAction::getModuleName(). "/AlunoAtividadeEnvios/";
    }
	...
	public static function getKeys($tipo = null) {
        ...
        foreach ($aCamposId as $o) {
            ...
            $aRet["A"][] = $attr;
            $aRet["F"][] = AlunoAtividadeEnviosAction::transformaColunas($attr);
        }
        if ($tipo == "F") {
	...
	public static function getFieldsStructure($field = null){
        ...
        $oAction = new AlunoAtividadeEnviosAction();
	...
	public static function maxlengthFields() {
        $aFields = AlunoAtividadeEnviosAction::getFieldsStructure();
        $aMaxlength = array();
